...
class ServicesDetailsForm(forms.ModelForm):
    ...
    class Meta:
		...
		ITEMS (donationID, description, isack_sent, approxValue)
Foreign key donationID references DONATIONS(donationID)

		
class ItemDonationForm(forms.ModelForm):
	ItemDescription     = forms.CharField(max_length=100)
	ACKstatus           = forms.CharField(max_length=2,required=False)
	DonatedItemValue	= forms.NumberField(max_value=100)
...
class MonetaryDonationForm(forms.ModelForm):
	...
	DonationValue	    = forms.NumberField(max_value=100)
